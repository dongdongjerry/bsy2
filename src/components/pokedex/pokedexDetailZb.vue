<template>
  <div class="pokedexDetail pokedexDetailZb">
    ...
    <div class="pokedexDetail-hd">
		...
		<div class="pokedexDetail-from">
			<ul>
				...
				<li>
					<label>套装属性</label>
					<div class="textarea" contenteditable="true"><pre>{{dataTzsx}}</pre></div>
					<textarea maxlength="300" v-model="dataTzsx" :disabled="isDisabled"></textarea>
				</li>
				<li>
					<label>列传</label>
				...
				<li>
					<label>天赋加成</label>
					<div class="textarea" contenteditable="true"><pre>{{dataTfjc}}</pre></div>
					<textarea maxlength="300" v-model="dataTfjc" :disabled="isDisabled"></textarea>
				</li>
			</ul>
		</div>
		<mt-popup
    ...
    export default {
    	...
    	created () {
    		...
    		this.$http.apiV0(`photostrategy/${this.id}/`).then(data => {
		    	...
		    	this.dataTzsx = this.pokedexDetailData.kvs['套装属性'];
		    	this.dataLz = this.pokedexDetailData.kvs['列传'];
		    	this.dataTfjc = this.pokedexDetailData.kvs['天赋加成'];
		    });
		    // 修改历史记录数据请求 v1
